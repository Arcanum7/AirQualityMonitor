...
class Tasks():
    ...
    def addAllTasks(self, tasksData):
        ...
        # update all files for VS Code so it can be used as IDE
        task = self.getResetAndRunTask()
        tasksData = self.addOrReplaceTask(tasksData, task)
    ...
    def getBuildTask(self):
        ...
        jsonTaskData["label"] = F"🔨{tmpStr.taskName_build}"
        ...
        parallelJobsNumber = int(numOfCores * 1.5)
        parallelJobsStr = "-j" + str(parallelJobsNumber)
        # set 'make' parallel job execution
        jsonTaskData["args"].append(parallelJobsStr)
    ...
    def getCompileTask(self):
        ...
        for flagIndex, flag in enumerate(cFlags):
            if flag == "-MF":
                newFlagString = "-MF'" + buildDir + \
                    "/${fileBasenameNoExtension}.d'"
    ...
    def getDeleteBuildFolderTask(self):
        ...
        buildData = build.BuildData().getBuildData()
        jsonTaskData["label"] = F"🔥{tmpStr.taskName_clean}"
        jsonTaskData["command"] = buildData[self.bStr.buildToolsPath]
    ...
    def getBuildDownloadAndRunTask(self):
        ...
        jsonTaskData["label"] = F"🚀{tmpStr.taskName_CPU_buildDownloadRun}"
    ...
    def getDownloadAndRunTask(self):
        ...
        programString = "program " + \
            buildData[self.bStr.targetExecutablePath] + " verify reset exit"
    ...
    def getResetAndRunTask(self):
        ...
        for arg in buildData[self.bStr.openOcdConfig]:
            jsonTaskData["args"].append("-f")
            jsonTaskData["args"].append(arg)
        # init must be executed before other commands!
        jsonTaskData["args"].append("-c init")
    ...
    def getOpenCubeMXTask(self):
        ...
        # opens with default program
        jsonTaskData["args"] = [utils.cubeMxProjectFilePath]
    ...
    def getSerialTask(self):
        ...
        buildData = build.BuildData().getBuildData()
        jsonTaskData = json.loads(taskData)
        jsonTaskData["label"] = F"📈{tmpStr.taskName_RunSerial}"
        jsonTaskData["command"] = buildData[self.bStr.pythonExec]
