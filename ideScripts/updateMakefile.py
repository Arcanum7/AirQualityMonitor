...
class Makefile():
            ...
            if self.hasPrintCapabilities(utils.makefilePath):
                ...
                if utils.pathExists(utils.makefileBackupPath):
                    ...
                    if self.hasPrintCapabilities(utils.makefileBackupPath):
                        ...
                        utils.copyAndRename(
                            utils.makefileBackupPath, utils.makefilePath)
                else:
    ...
    def getMakefileData(self, makeExePath, gccExePath):
        ...
        # project name
        projectName = self.getMakefileVariable(
            makeExePath, gccExePath, self.mkfStr.projectName)[0]
        dataDictionaryList[self.mkfStr.projectName] = projectName

        # dir name
        buildDirName = self.getMakefileVariable(
            makeExePath, gccExePath, self.mkfStr.buildDir)[0]
        dataDictionaryList[self.mkfStr.buildDir] = buildDirName

        # source files
        cSourcesList = self.getMakefileVariable(
            makeExePath, gccExePath, self.mkfStr.cSources)
        dataDictionaryList[self.mkfStr.cSources] = cSourcesList

        asmSourcesList = self.getMakefileVariable(
            makeExePath, gccExePath, self.mkfStr.asmSources)
        dataDictionaryList[self.mkfStr.asmSources] = asmSourcesList

        ldSourcesList = self.getMakefileVariable(
            makeExePath, gccExePath, self.mkfStr.ldSources)
            ...
            if startCharacter != NOT_FOUND:  # search for start string
                ...
                if line.find(endOfLineChars) == NOT_FOUND:
                    ...
                    if len(line) != 0:  # check for 'SOMETHING = ' (empty line after '=')
                        ...
                        # strip string of start and and characters
                        items.append(line)
                    return items
    ...
    def createNewMakefile(self):
        ...
        # sources
        cSources = cP.getCPropertiesKeyData(
            cPropertiesData, self.cPStr.user_cSources)
        data = self.searchAndAppend(data, self.mkfStr.cSources, cSources)

        asmSources = cP.getCPropertiesKeyData(
            cPropertiesData, self.cPStr.user_asmSources)
        data = self.searchAndAppend(data, self.mkfStr.asmSources, asmSources)

        ldSources = cP.getCPropertiesKeyData(
            cPropertiesData, self.cPStr.user_ldSources)
        data = self.searchAndAppend(
            data, self.mkfStr.ldSources, ldSources, preappend='-l:')

        # includes
        cIncludes = cP.getCPropertiesKeyData(
            cPropertiesData, self.cPStr.user_cIncludes)
        data = self.searchAndAppend(
            data, self.mkfStr.cIncludes, cIncludes, preappend='-I')

        asmIncludes = cP.getCPropertiesKeyData(
            cPropertiesData, self.cPStr.user_asmIncludes)
        data = self.searchAndAppend(
            data, self.mkfStr.asmIncludes, asmIncludes, preappend='-I')

        ldIncludes = cP.getCPropertiesKeyData(
            cPropertiesData, self.cPStr.user_ldIncludes)
        data = self.searchAndAppend(
            data, self.mkfStr.ldIncludes, ldIncludes, preappend='-L')

        # defines
        cDefines = cP.getCPropertiesKeyData(
            cPropertiesData, self.cPStr.user_cDefines)
        data = self.searchAndAppend(
            data, self.mkfStr.cDefines, cDefines, preappend='-D')

        asmDefines = cP.getCPropertiesKeyData(
            cPropertiesData, self.cPStr.user_asmDefines)
        data = self.searchAndAppend(
            data, self.mkfStr.asmDefines, asmDefines, preappend='-D')

        # compiler flags
        cFlags = cP.getCPropertiesKeyData(
            cPropertiesData, self.cPStr.user_cFlags)
        data = self.searchAndAppend(data, self.mkfStr.cFlags, cFlags)

        asmFlags = cP.getCPropertiesKeyData(
            cPropertiesData, self.cPStr.user_asmFlags)
        data = self.searchAndAppend(data, self.mkfStr.asmFlags, asmFlags)

        ldFlags = cP.getCPropertiesKeyData(
            cPropertiesData, self.cPStr.user_ldFlags)
            ...
            if line.find(searchString) != NOT_FOUND:  # search for start string
                ...
                if line.find("\\") == NOT_FOUND:
                        ...
                        if appendData:  # and it is not empty
                            if len(appendData) == 1:  # this list has only one item, add it without '\'
                                ...
                                for itemIndex, item in enumerate(appendData):
                                    stringToInsert = item
                                    # for last item do not append "\"
                                    if item != appendData[-1]:
                                        stringToInsert += "\\"
                                    stringToInsert += "\n"  # new line must always be added
                                    data.insert(
                                        lineIndex + itemIndex + 1, stringToInsert)
                    ...
                    if isinstance(appendData, list):
                        for itemIndex, item in enumerate(appendData):
                            stringToInsert = item + " \\\n"
                            data.insert(lineIndex + itemIndex +
                                        1, stringToInsert)
                    else:  # appendData is string (not list)
                        data[lineIndex] += item + " \\\n"
                ...
                if line.find("\\") == NOT_FOUND:
                    ...
                    while data[lastLineIndex].rstrip('\n') != '':
                        ...
                        if lastLineIndex >= len(data):
                            errorMsg = "Unable to find end of multi-line Makefile item (" + \
                                searchString + "). "
                            errorMsg += "Was Makefile manually modified?"
                            utils.printAndQuit(errorMsg)
                    # delete this lines
                    delLineIndex = lineIndex + 1
                    # this line will be deleted until an empty line is present
                    constLineIndex = lineIndex + 1
    ...
    def getMakefileVariable(self, makeExePath, gccExePath, variableName):
        ...
        if returnString.find(tmpStr.printMakefileDefaultString) != -1:
            returnString = returnString.replace(
                tmpStr.printMakefileDefaultString, '')

        # split string to list and remove empty items
        returnStringList = returnString.split(' ')
    ...
    def replaceMakefileHeader(self, data):
        ...
        lastLine = None
        for lineIndex, line in enumerate(data):
            # first line is ######... and second should be '# target'
            twoLinesAhead = data[lineIndex + 2]
    ...
    def addMakefileCustomFunctions(self, pathToMakefile):
        ...
        with open(pathToMakefile, 'r+') as makefile:
            ...
            makefileDataLines = self.addPrintVariableFunction(
                makefileDataLines)
